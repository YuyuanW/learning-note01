const link = (url)=>{
    let link1 = '';
    if (url.indexOf('www') !== -1) link1 = url.split('www.')[1];
    else if (url.indexOf('www') === -1) link1 = url.split('//')[1];
    if (link1.indexOf('/') !== -1) link1 = link1.split('/')[0];
    return link1;
};
...
$('.addButton').on('click', ()=>{
    ...
    if (urlNew.indexOf(true)) urlNew = 'https://' + urlNew;
    hashMap.push({
        url: urlNew
    });
    render();
